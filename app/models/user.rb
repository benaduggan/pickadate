class User < ActiveRecord::Base
   ...
   validates :username, presence: true, uniqueness: true, length: { maximum: 100 }, :format => { :with => VALID_USERNAME_REGEX }
    ...
    def self.from_omniauth(auth)
      if auth #hash that comes back from Facebook, this is so we can do both facebook and normal users through same sessions controller
        where(auth.slice(:provider, :uid)).first_or_initialize.tap do |user|
          ...
          user.password = 'hAhQQBcJL2QkQuz'
          user.password_confirmation = 'hAhQQBcJL2QkQuz'
